...
- [x] **Updated April 26, 2026** — Revised session/summary visibility model
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /sessions/{sessionId} {
      // User can read ONLY while session is active or closing.
      // Once closed, the record is locked — only the next assigned volunteer
      // (Figma prototype) and professionals retain read access.
      allow read: if (request.auth != null
                      && request.auth.uid == resource.data.userId
                      && resource.data.status in ['active', 'closing'])
                  || isProfessional();
                  // NOTE: Next assigned volunteer read access is enforced
                  // in the Figma prototype volunteer interface only.
                  // Add: || isAssignedVolunteer(sessionId)  when building it.
    ...
    match /summaries/{summaryId} {
      // User CANNOT read their own summaries.
      // The summary exists for the next volunteer's context and professional oversight.
      // User retains write access ONLY to volunteerRating and volunteerFeedback fields.
      allow read: if isProfessional();
                  // NOTE: Next assigned volunteer read access (Figma prototype only).
                  // Add: || isAssignedVolunteer(resource.data.sessionId)  when building it.
      allow create: if request.auth != null
                    && request.auth.uid == request.resource.data.userId;
      allow update: if request.auth != null
                    && request.auth.uid == resource.data.userId
                    && request.resource.data.diff(resource.data).affectedKeys()
                        .hasOnly(['volunteerRating', 'volunteerFeedback']);
    }
    ...
    match /professionals/{profId} {
      // Any authenticated user can read professionals.
      // City and availability filtering is done in the app query, not enforced here.
      allow read: if request.auth != null;
      allow write: if false;
    }
...
| Session messages | Owner only during active session (deleted on close) |
| Session record | Owner read during `active`/`closing` only; after close → next assigned volunteer (Figma) + professionals |
| Summary | No user read; owner write limited to rating + feedback fields only; next assigned volunteer (Figma) + professionals |
| Volunteer profiles | Any authenticated user (read) |
| Hospital data | Public (no auth required) |
| Professionals list | Any authenticated user — filtered by city + availability in query |
